...
causalForest <- function(formula, data, treatment,
                         ...
                         cost=F, weights=F, num.cores = 1, seed = 1234) {
...
###    if (verbose) # removed from parameters
###    {
...
###  }
    ...
    hon_trees_list   <-   parallel::mclapply(1:num.trees, function(tree.index)
    {
        ...
        tree.honest <- honest.causalTree(formula=formula, data = dataTree,
                                         ...
                                         split.alpha = 0.5, cv.alpha = 0.5, xval=0,
                                         HonestSampleSize=est.size, cp=0)
        list(honest_tree   =   tree.honest,
             full_index    =   full.idx)
    }, mc.cores = num.cores)
    causalForest.hon$trees   <-   lapply(hon_trees_list, function(x) x$honest_tree)


    for (tree.index in 1:num.trees) {
        full.idx <- hon_trees_list[[tree.index]]$full_index
        causalForest.hon$inbag[full.idx, tree.index] <- 1
    }
